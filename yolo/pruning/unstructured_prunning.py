...
for idx, (name, module) in enumerate(conv_layers):
    if idx < skip_first or idx >= len(conv_layers) - skip_last:
        ...
        continue
    print(f"Aplicando pruning em {name}")
    prune.l1_unstructured(module, name='weight', amount=0.6) # Aplicando 60% de pruning não estruturado
...
print("=" * 60)
print("ESTATÍSTICAS APÓS PRUNING (EM MEMÓRIA)")
print("=" * 60)
...
print(f"Redução teórica: {((total_params_orig - nonzero_params_pruned)/total_params_orig)*100:.2f}%")
print()

# Salva o modelo com pruning aplicado
output_path = 'test.pt'
model.save(output_path)

# Verifica tamanho final do arquivo salvo
print("=" * 60)
...
print(f"Modelo salvo com pruning seletivo aplicado em '{output_path}'")
print("=" * 60)